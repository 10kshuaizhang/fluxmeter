...
echo "==> Checking OpenAPI 4.7 metering proof contract"
grep -q 'version: 4.7' "$OPENAPI"
...
grep -q 'enum: \[accepted, partial, conflict, failed, overloaded\]' "$OPENAPI"
grep -q 'enum: \[accepted, quarantined, conflict, rejected, pending, uncertain, unavailable, overloaded\]' "$OPENAPI"
grep -q 'custody_overloaded' "$OPENAPI"
