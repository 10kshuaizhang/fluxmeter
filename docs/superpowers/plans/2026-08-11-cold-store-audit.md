...
- Ship as **3.3.0** (MINOR — new cold-store capability) when implementation lands; update `build.gradle`, `changLog.md`, `progress.md`, `ROADMAP.md` in the final task.
...
| `build.gradle` / `changLog.md` / `progress.md` / `ROADMAP.md` | Version 3.3.0 + checklist |
...
**Files:**
- Modify: `build.gradle` (`version = '3.3.0'`)
- Modify: `changLog.md` (new `[3.3.0]` section)
...
- `progress.md`: Current version `3.3.0`; ADR-1 row **Done**; Recent Activity line for 3.3.0.
- `ROADMAP.md`: engine version `3.3.0`; ecosystem track cold store marked shipped/spec implemented.
- Spec header: `Status: APPROVED — implemented in 3.3.0`.
...
git commit -m "chore: release 3.3.0 auditable ClickHouse cold store"
```
