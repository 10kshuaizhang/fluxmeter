"""Usage bucket + session read tests (requires local Redis)."""
...
class TestUsageBuckets:
    ...
    def test_session_read(self, r):
        sid = f"sess_{uuid.uuid4().hex[:8]}"
        cid = f"cust_{uuid.uuid4().hex[:8]}"
        r.set(f"session:{sid}:customer_id", cid)
        r.set(f"session:{sid}:input_tokens", "100")
        r.set(f"session:{sid}:output_tokens", "50")
        r.set(f"session:{sid}:total_tokens", "150")
        r.set(f"session:{sid}:event_count", "1")
        r.set(f"session:{sid}:cost_usd", "0.01")
